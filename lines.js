...
function setup(){
  ...
  for(let i=0;i<20;i++){
    ...
    let brEak=0;
    lines.push(new crawlingLines(x,y,spEed,x_lo,x_hi,skewX,y_lo,y_hi,skewY,x_target,y_target,x_inc,y_inc,brEak));
    lines[i].display();
    print(lines[i].x, lines[i].y);
  }
}
...
class crawlingLines{
  constructor(x,y,spEed,x_lo,x_hi,skewX,y_lo,y_hi,skewY,x_target,y_target,x_inc,y_inc,brEak){
    this.x=x;
    this.y=y;
    this.spEed=spEed;
    this.x_lo=x_lo;
